...
public class Util {
	...
	public static boolean dateIsBetween(Date date, Date range1, Date range2, boolean inc) {
		if (inc)
	...
	public static String formatName(Student stu) {
		return stu.getFirstName() + " " + stu.getLastName() + " - " + stu.getID();
	}
	...
	public static Student getStudent(String ID) {
		if (!idIsTaken(ID))
			throw new RuntimeException("Can't get student, ID doesn't exist");
		for (Student stu : Student.ALLSTUDENTS)
			if (stu.getID().equals(ID))
				return stu;
		return null;

...
public class MainView extends View implements MouseListener {
	...
	private final ActionListener enterButtonListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (idattempt.isEmpty())
				...
				JOptionPane.showMessageDialog(MainView.this, "That ID is not in the system", "Student does not Exist",
						JOptionPane.ERROR_MESSAGE, null);
				return;
			}
			ViewCnfg cnfg = new ViewCnfg();
			cnfg.admin = false;
			cnfg.stu = Util.getStudent(idattempt);
	...
	public MainView(FGFrame parent) {
		...
		loginLabel = new JLabel("<html>&nbsp&nbsp Login with Student ID: </html>");

		loginField = new HintTextField("Enter ID Here...");
		loginField.setToolTipText("Enter Student ID");
		...
		loginButton.setToolTipText("Log in to Student Page");
	...
	public void cnfgFor(ViewCnfg cnfg) {
		...
		Image img = Util.getImage("res_life.jpg").getImage();

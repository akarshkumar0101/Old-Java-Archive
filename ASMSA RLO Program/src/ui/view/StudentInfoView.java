...
public class StudentInfoView extends View {
	...
	private class ContentPanel extends JScrollPane {
		...
		// 0 for creating new
		// 1 for admin editing stu
		// 2 for non editable stu show (admin or not)
		int type = -1;
		...
		private final ActionListener createVolListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Student newstu = createNewVolunteer();
				if (newstu == null)
					return;
				Student.ALLSTUDENTS.add(newstu);

				viewcnfg.stu = newstu;
				createApplyButton.setEnabled(false);
				parent.replaceCurrentView(StudentInfoView.this, viewcnfg);
		...
		public ContentPanel() {
			...
			if (viewcnfg.stu == null && viewcnfg.admin) {
				// creating new stu
				type = 0;
			...
			} else if (viewcnfg.stu != null && viewcnfg.admin && viewcnfg.editnow) {
				// admin editing
				type = 1;
				nameField = new JTextField();
				idField = new JTextField();
			...
			phoneField.addFocusListener(new FocusListener() {
				...
				public void focusLost(FocusEvent e) {
					try {
						...
						// JOptionPane.showMessageDialog(StudentInfoView.content,
			...
			if (type > 0) {
				...
				totalHoursButton.setText("<html><div style=\"text-align: center;\">Total Hours:<br>"
						+ Util.formatTime(viewcnfg.stu.getTotalVolunteeringTime()) + "</html>");
			...
			createApplyButton
					.setText((viewcnfg.stu == null) ? "Create New Student" : "<html>Save Changes<br>to Student</html>");
			createApplyButton.setToolTipText((viewcnfg.stu == null) ? "Enter the new volunteer to the database"
					: "<html>Save all changes made to the record</html>");

			if (type == 0) {
				createApplyButton.setText("Create New Student");
				...
				createApplyButton.setText("<html>Save Changes<br>to Student</html>");
	...
	public StudentInfoView(FGFrame parent) {
		super(parent);
	...
	/**
	 * Configures the StudentInfoView for either a new Student(admin=true, stu=null)
	 * or an existing one(stu!=null)
	 */
	@Override
	public void cnfgFor(ViewCnfg cnfg) {
		...
		if (cnfg.stu == null) {
			titleLabel.setText("New Student");
			...
			if (cnfg.admin) {
				...
				subLabel.setText(Util.formatName(cnfg.stu));
			} else {
				titleLabel.setText(cnfg.stu.getName());
				subLabel.setText(cnfg.stu.getID());
			}
		}
	...
	public void beforeClose() {
		if (contentPanel.type == 2) {
			...
			int result = JOptionPane.showConfirmDialog(this, "Would you like to cancel creating the new Student?",
					"Student Creation Canceling", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null);
	...
	public void afterDisplay() {
		...
		if (contentPanel.type == 0) {// creating new
			...
			contentPanel.nameField.setText(viewcnfg.stu.getName());

			contentPanel.chooseIDrb.doClick();
			contentPanel.idField.setText(viewcnfg.stu.getID());

			(viewcnfg.stu.getGender() == Student.MALE ? contentPanel.malerb : contentPanel.femalerb).doClick();

			contentPanel.ageBox.setSelectedIndex(viewcnfg.stu.getAge());

			contentPanel.phoneField.setText(viewcnfg.stu.getPhoneNumber());
			contentPanel.emailField.setText(viewcnfg.stu.getEmail());
			contentPanel.addressField.setText(viewcnfg.stu.getAddress());

			contentPanel.createApplyButton.setEnabled(true);
		} else if (contentPanel.type == 2) {// no edit
			contentPanel.nameField.setText(viewcnfg.stu.getName());
			contentPanel.idField.setText(viewcnfg.stu.getID());
			contentPanel.genderField.setText(viewcnfg.stu.getGender() == Student.MALE ? "Male" : "Female");
			contentPanel.ageFieldLabel.setText(viewcnfg.stu.getAge() + "");
			contentPanel.phoneField.setText(viewcnfg.stu.getPhoneNumber());
			contentPanel.emailField.setText(viewcnfg.stu.getEmail());
			contentPanel.addressField.setText(viewcnfg.stu.getAddress());
	 ...
	 * @return
	 */
	private Student createNewVolunteer() {
		...
		if (contentPanel.ageBox.getSelectedIndex() == 0) {
			tellLeftBlank(StudentInfoView.this);
			return null;
		}

		String[] name = contentPanel.nameField.getText().split(" ");
		Student stu = new Student(name[0], name[1], id, null);
		stu.setGender(contentPanel.malerb.isSelected() ? Student.MALE : Student.FEMALE);
		stu.setAge(contentPanel.ageBox.getSelectedIndex());
		String phone = contentPanel.phoneField.getText();
		stu.setPhoneNumber((phone.equals("") ? "Not given" : phone));
		String email = contentPanel.emailField.getText();
		stu.setEmail((email.equals("") ? "Not given" : email));
		String address = contentPanel.addressField.getText();
		stu.setAddress((address.equals("") ? "Not given" : address));
	...
	public boolean saveAllChanges() {
		...
		if (contentPanel.phonenumIDrb.isSelected()) {
			...
			try {
				pn = pn.substring(pn.length() - 4);
				if (Util.idIsTaken(pn) && !id.equals(viewcnfg.stu.getID())) {
					tellIDIsTaken(this, pn);
					return true;
				}
				id = pn;
		...
		if (contentPanel.chooseIDrb.isSelected() && id.equals("")) {
			tellLeftBlank(this);
			return true;
		} else if (Util.idIsTaken(id) && !id.equals(viewcnfg.stu.getID())) {
			tellIDIsTaken(this, id);
			return true;
		}
		...
		viewcnfg.stu.setName(contentPanel.nameField.getText());
		viewcnfg.stu.setID(id);
		viewcnfg.stu.setGender(contentPanel.malerb.isSelected() ? Student.MALE : Student.FEMALE);
		viewcnfg.stu.setAge(contentPanel.ageBox.getSelectedIndex());
		String phone = contentPanel.phoneField.getText();
		viewcnfg.stu.setPhoneNumber(phone.equals("") ? "Not given" : phone);
		String email = contentPanel.emailField.getText();
		viewcnfg.stu.setEmail(email.equals("") ? "Not given" : email);
		String address = contentPanel.addressField.getText();
		viewcnfg.stu.setAddress(address.equals("") ? "Not given" : address);
	...
	private static void tellIDIsTaken(StudentInfoView view, String id) {
	...
	private static void tellLeftBlank(StudentInfoView view) {
	...
	public String getFrameTitleString() {
		return (viewcnfg.stu == null ? "Creating New Student"
				: "Student Information for " + Util.formatName(viewcnfg.stu));
	}

	@Override
	public String getBackString() {
		return (viewcnfg.stu == null ? "Creating New Student" : "Student Information");
	}

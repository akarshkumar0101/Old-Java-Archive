...
public class TimeChartView extends View {
	...
	private class TimeTable extends JPanel {
			...
			if (viewcnfg.admin) {
				...
				applyButton.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						System.out.println(viewcnfg.stu.isSignedIn());
					}
				});
		...
		public void setupSessionRows() {
			sessionRows.clear();
			for (int i = 0; i < viewcnfg.stu.getSessions().size(); i++) {
				sessionRows.add(new SessionRow(viewcnfg.stu.getSessions().get(i), i + 1));
			}
			System.gc();
		}
		...
		public void initTotalHeader() {
			...
			ttgbc.weightx = weight_time;
			ttgbc.gridx = 3;
			String totaltimestring = Util.formatTime(viewcnfg.stu.getTotalVolunteeringTime());
	...
	public void cnfgFor(ViewCnfg cnfg) {
		...
		volButton.setText(Util.formatName(viewcnfg.stu));
		...
		Util.setActionListener(volButton, new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (viewcnfg.admin) {
					...
					parent.showNextView(parent.stuview, cnfg);
				}
			}
		});
	...
	public boolean applyAllChanges() {
		...
		try {
			...
			viewcnfg.stu.getSessions().clear();

			for (TimeTable.SessionRow sr : timeTable.sessionRows) {
				Date in = Util.normalFormat.parse(sr.signinField.getText());
				Date out = Util.normalFormat.parse(sr.signoutField.getText());

				viewcnfg.stu.getSessions().add(new Session(in, out));
			}

			viewcnfg.stu.sortSessions();
			DataController.saveStudent(viewcnfg.stu);
			return false;
	...
	public String getFrameTitleString() {
		return "Time Chart for " + Util.formatName(viewcnfg.stu);
	}

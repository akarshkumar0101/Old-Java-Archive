...
public class AdminView extends View {
	...
	private final ActionListener addVolListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			cnfg.admin = true;
			parent.showNextView(parent.stuinfoview, cnfg);
		}
	};
	...
	private class ContentPanel extends JPanel {
		...
		private final ArrayList<StudentLabel> volLabels = new ArrayList<StudentLabel>();
		...
		private class StudentLabel {
			...
			private final ActionListener infoButtonListener = new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					viewcnfg.stu = stu;
					parent.showNextView(parent.stuinfoview, viewcnfg);
				}
			};
			private final ActionListener removeButtonListener = new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					int result = JOptionPane.showConfirmDialog(AdminView.this,
							"Are you sure you want to permanently delete " + Util.formatName(stu) + "?",
							"Permanently Delete Student?", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE,
							null);

					if (result == JOptionPane.NO_OPTION)
						return;
					else if (result == JOptionPane.YES_OPTION) {
						Student.ALLSTUDENTS.remove(stu);
						DataController.deleteStudent(stu);
			...
			private StudentLabel(Student stu) {
				super();
				this.stu = stu;
				...
				nameLabel.setText(Util.formatName(stu));
				nameLabel.setHorizontalAlignment(SwingConstants.CENTER);
				nameLabel.setFont(Util.standoutFont);

				inButton = new JButton("Sign in");
				inButton.setToolTipText("Manually sign " + Util.formatName(stu) + " in");
				outButton = new JButton("Sign out");
				outButton.setToolTipText("Manually sign " + Util.formatName(stu) + " out");
				...
				infoButton.setToolTipText("See volunteer information for " + Util.formatName(stu));
				infoButton.setIcon(usericon);
				Util.setActionListener(infoButton, infoButtonListener);

				removeButton = new JButton();
				removeButton.setToolTipText("Permanently delete " + Util.formatName(stu) + " from the system");
			...
			private void updateText() {
				if (stu.isSignedIn()) {
					...
					lastAffLabel.setText("<html><div style=\"text-align: center;\">Last Signed in at:<br>"
							+ Util.normalFormat.format(stu.lastAffiliation()) + "</html>");
					...
					Session lastsess = stu.getLastSession();
		...
		private ContentPanel() {
			...
			JLabel volTitle = new JLabel("<html><div style=\"text-align: center;\">Student Name - ID</html>");
			...
			JLabel volinfoTitle = new JLabel("<html><div style=\"text-align: center;\">Student<br>Info</html>");
			...
			JLabel deletevolTitle = new JLabel("<html><div style=\"text-align: center;\">Delete<br>Student</html>");
			...
			for (Student stu : Student.ALLSTUDENTS) {
				cpgbc.gridwidth = 1;
				StudentLabel vl = new StudentLabel(stu);
				volLabels.add(vl);
	...
	public void cnfgFor(ViewCnfg cnfg) {
		...
		if (!cnfg.admin || cnfg.stu != null)
			throw new RuntimeException("Student or non admin trying to access admin view");

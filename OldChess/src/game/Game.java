...
public class Game {
	...
	public void setupAllButtonsAndLabels() {
		...
		ImageIcon icon = new ImageIcon(
				"C:\\Users\\Akarsh\\Documents\\Eclipse\\Chess\\src\\graphics\\Chess Icons\\BackButton.png");
		...
		for (int y = 0; y < 8; y++) {
			for (int x = 0; x < 8; x++) {
				// buttons[x][y] = new MyButton(currentBoard.getPiece(new
				// Coordinate(x,y)).toString(), new Coordinate(x,y));
				buttons[x][y] = new MyButton(currentBoard.getPiece(new Coordinate(x, y)).getImage(),
						new Coordinate(x, y));
				buttons[x][y].setToolTipText(x + "," + y);

				if ((y % 2 == 0 && x % 2 == 1) || (y % 2 == 1 && x % 2 == 0)) {
					buttons[x][y].setBackground(DARK_SQUARE_COLOR);
				} else {
					buttons[x][y].setBackground(LIGHT_SQUARE_COLOR);
				}
			}
		}
	...
	public void updateAll() {
		for (int y = 0; y < 8; y++) {
			for (int x = 0; x < 8; x++) {
				// buttons[x][y].setText(currentBoard.getPiece(new
				// Coordinate(x,y)).toString());
		...
		for (int x = 0; x < 8; x++) {
			for (int y = 0; y < 8; y++) {
				if (((y % 2 == 1 && x % 2 == 0) || (y % 2 == 0 && x % 2 == 1))
						&& buttons[x][y].getBackground() != DARK_SQUARE_COLOR) {
					highlight(new Coordinate(x, y), DARK_SQUARE_COLOR, false);
				}
				if (((y % 2 == 0 && x % 2 == 0) || (y % 2 == 1 && x % 2 == 1))
						&& buttons[x][y].getBackground() != LIGHT_SQUARE_COLOR) {
	...
	public void pickCoordinate(Coordinate coordinatePicked) {
		...
		if (firstPartOfInput) {
			...
			if (currentBoard.getPiece(coordinatePicked).getClass() != NoPiece.class
					&& currentBoard.getPiece(coordinatePicked).getPlayer() == playerTurn) {
				...
				if (checkIfCheckMate(currentBoard, true) || checkIfCheckMate(currentBoard, false)) {
					if (winner == 1 || winner == 2) {
						...
						if (winner == 1) {
							...
							JOptionPane.showMessageDialog(frame, label, "Winner!", JOptionPane.INFORMATION_MESSAGE,
									ChessPiece.getImage(new King(new Coordinate(0, 0), true)));
						}
						if (winner == 2) {
							unhighlightAll();
							...
							JOptionPane.showMessageDialog(frame, label, "Winner!", JOptionPane.INFORMATION_MESSAGE,
									ChessPiece.getImage(new King(new Coordinate(0, 0), false)));
						}
						frame.dispose();
	...
	public boolean isValidMove(Board board, Coordinate movingFrom, Coordinate movingTo) {

		if (board.getPiece(movingFrom).getClass() == NoPiece.class)
			return false;

		if (board.getPiece(movingFrom).getPlayer() == board.getPiece(movingTo).getPlayer()
				&& board.getPiece(movingTo).getClass() != NoPiece.class)
			return false;

		if (board.getPiece(movingFrom).getClass() == Pawn.class)
			return Pawn.isValidMove(board, movingFrom, movingTo);
		if (board.getPiece(movingFrom).getClass() == Rook.class)
			return Rook.isValidMove(board, movingFrom, movingTo);
		if (board.getPiece(movingFrom).getClass() == Bishop.class)
			return Bishop.isValidMove(board, movingFrom, movingTo);
		if (board.getPiece(movingFrom).getClass() == Knight.class)
			return Knight.isValidMove(board, movingFrom, movingTo);
		if (board.getPiece(movingFrom).getClass() == Queen.class)
			return Queen.isValidMove(board, movingFrom, movingTo);
		if (board.getPiece(movingFrom).getClass() == King.class)
			return King.isValidMove(board, movingFrom, movingTo);
	...
	public boolean isActualValidMove(Board board, Coordinate movingFrom, Coordinate movingTo) {
		if (isValidMove(board, movingFrom, movingTo)) {
			...
			if (!checkIfChecked(tempBoard, board.getPiece(movingFrom).getPlayer()))
				return true;
		}
		return false;
	}
	...
	public void checkAndUpdatePawnStatus() {
		for (int x = 0; x < 8; x++) {
			if (currentBoard.getPiece(new Coordinate(x, 0)).getClass() == Pawn.class
					&& !currentBoard.getPiece(new Coordinate(x, 0)).getPlayer()) {
				currentBoard.setPiece(new Queen(new Coordinate(x, 0), false));
			}
			if (currentBoard.getPiece(new Coordinate(x, 7)).getClass() == Pawn.class
					&& currentBoard.getPiece(new Coordinate(x, 7)).getPlayer()) {
	...
	public boolean checkIfChecked(Board board, boolean player) {
		...
		for (int y = 0; y < 8; y++) {
			for (int x = 0; x < 8; x++) {
				if (board.getPiece(new Coordinate(x, y)).getClass() == King.class
						&& board.getPiece(new Coordinate(x, y)).getPlayer() == player) {
					king.set(x, y);
				}
			}
		}
	...
	public boolean checkIfCheckMate(Board board, boolean player) {
		...
		if (!checkIfChecked(board, player))
			return false;

		for (int y1 = 0; y1 < 8; y1++) {
			for (int x1 = 0; x1 < 8; x1++) {

				if (board.getPiece(new Coordinate(x1, y1)).getPlayer() == player
						&& board.getPiece(new Coordinate(x1, y1)).getClass() != NoPiece.class) {

...
public final class Application {
	public static final Image appImage = Util.getImage("freegeek_logo.png");
	...
	public static boolean shouldSaveAppPrefs = false;

	public static final Thread onCloseThread = new Thread(() -> {
		for (Volunteer vol : Volunteer.volunteers) {
			if (vol.isSignedIn()) {
				vol.signout();
			}
		}
		if (shouldSaveAppPrefs) {
			try {
	...
	public static final void firstTimeStart() {
		...
		final Runnable afterFirstTimeSetup = () -> {
			try {
				dialog.dispose();
				startFGFrame();
			} catch (Exception e) {

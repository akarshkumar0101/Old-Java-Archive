...
public class TimeChartView extends View {
	...
	private class TimeTable extends JPanel {
		...
		private final Image addImage = Util.getScaledImage("add.png", picSize, picSize);
		private final Image deleteImage = Util.getScaledImage("delete.png", picSize, picSize);
		private final DocumentListener changeListener=new DocumentListener(){@Override public void insertUpdate(DocumentEvent e){if(!applyButton.isEnabled()){applyButton.setEnabled(true);}}@Override public void removeUpdate(DocumentEvent e){if(!applyButton.isEnabled()){applyButton.setEnabled(true);}}@Override public void changedUpdate(DocumentEvent e){}};
		...
		public class SessionRow {
			...
			private SessionRow(Session s, int y) {
				...
				if (viewcnfg.admin) {
					removeButton = new JButton(new ImageIcon(deleteImage));
		...
		public TimeTable() {			super();			scrollPane = new JScrollPane();			table = new JPanel();			scrollPane.setViewportView(table);			if (viewcnfg.admin) {				applyButton = new JButton("<html><div style=\"text-align: center;\">Save<br>Changes</html>");				applyButton.setMargin(new Insets(5, 5, 5, 5));				applyButton.addActionListener(new ActionListener() {					@Override					public void actionPerformed(ActionEvent e) {						boolean crash = applyAllChanges();						applyButton.setEnabled(crash);						if (crash)							return;						setupSessionRows();						setupAll();						updateEverything();						centertopit();						System.out.println(viewcnfg.vol.isSignedIn());					}				});				applyButton.setEnabled(false);				applyButton.setToolTipText("Save all changes made to the record");				addButton = new JButton(new ImageIcon(addImage));				addButton.addActionListener(new ActionListener() {					@Override					public void actionPerformed(ActionEvent e) {						sessionRows.add(new SessionRow(new Session(), sessionRows.size() + 1));						setupAll();						updateEverything();						centertopit();						if (!applyButton.isEnabled()) {							applyButton.setEnabled(true);						}					}				});				addButton.setToolTipText("Add a new session manually");			}			sessionRows = new ArrayList<>();			setupSessionRows();			setupAll();			setLayout(new BorderLayout());			addComponentListener(new ComponentListener() {				boolean previouslybigger = true;
	...
	JScrollBar horbar = scrollPane
			.getHorizontalScrollBar();horbar.setPreferredSize(new Dimension(0,30));horbar.setUnitIncrement(10);
	JScrollBar bar = scrollPane
			.getVerticalScrollBar();bar.setPreferredSize(new Dimension(30,0));bar.setUnitIncrement(10);
	InputMap im = bar.getInputMap(
			JComponent.WHEN_IN_FOCUSED_WINDOW);im.put(KeyStroke.getKeyStroke("down"),"positiveUnitIncrement");im.put(KeyStroke.getKeyStroke("up"),"negativeUnitIncrement");scrollPane.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS);

	setBorder(null);			scrollPane.setBorder(null);		}

	public void setupSessionRows() {			sessionRows.clear();			for (int i = 0; i < viewcnfg.vol.getSessions().size(); i++) {				sessionRows.add(new SessionRow(viewcnfg.vol.getSessions().get(i), i + 1));			}			System.gc();		}
	...
	public void initTotalHeader() {
		...
		totaltime.setToolTipText("Total career employeeing time");
	...
	public void cnfgFor(ViewCnfg cnfg) {
		...
		volButton.setToolTipText(cnfg.admin ? "See employee information" : "See your account");
	...
	public void scrollDown() {		SwingUtilities.invokeLater(() -> {				JScrollBar bar = timeTable.scrollPane.getVerticalScrollBar();				bar.setValue(bar.getMaximum());			});	}

public boolean applyAllChanges() {		int crashAt = 1;		String crashMessage = "";		Exception crash = null;		try {			// check for crash			for (TimeTable.SessionRow sr : timeTable.sessionRows) {				Session.checkTimes(Util.normalFormat.parse(sr.signinField.getText()),						Util.normalFormat.parse(sr.signoutField.getText()));				crashAt++;			}			viewcnfg.vol.getSessions().clear();			for (TimeTable.SessionRow sr : timeTable.sessionRows) {				Date in = Util.normalFormat.parse(sr.signinField.getText());				Date out = Util.normalFormat.parse(sr.signoutField.getText());				viewcnfg.vol.getSessions().add(new Session(in, out));			}			viewcnfg.vol.sortSessions();			DataController.saveVol(viewcnfg.vol);			return false;		} catch (ParseException e) {			crashMessage = "<html><div style=\"text-align: center;\"><strong>Invalid time format at session " + crashAt					+ "</strong><br><br>Time has to be in the format of <br>[2/25/2016 12:39 AM]</html>";			crash = e;		} catch (IllegalArgumentException e) {			crashMessage = "<html><div style=\"text-align: center;\"><strong>Invalid times at session " + crashAt					+ "</strong><br><br>Sign out time has to come after sign in time</html>";			crash = e;		}		JOptionPane.showMessageDialog(this, crashMessage, crash.getClass().getSimpleName(), JOptionPane.ERROR_MESSAGE,				null);		return true;	}	@Override	public void afterDisplay() {		timeTable.centertopit();		scrollDown();	}	@Override	public void saveMem() {		timeTable = null;	}	@Override	public void beforeClose() {		if (timeTable == null)			return;		if (!viewcnfg.admin || !timeTable.applyButton.isEnabled()) {			this.remove(timeTable);			return;		}		int result = JOptionPane.showConfirmDialog(this, "Would you like to save changes?", "Unsaved Changes",				JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null);		if (result == JOptionPane.YES_OPTION) {			boolean crash = applyAllChanges();			if (crash) {				parent.cancelTransition();				return;			}		} else if (result == JOptionPane.CANCEL_OPTION) {			parent.cancelTransition();			return;		}		this.remove(timeTable);	}	@Override	public String getFrameTitleString() {		return "Time Chart for " + Util.formatName(viewcnfg.vol);	}	@Override	public String getBackString() {		return "Time Chart";	}}
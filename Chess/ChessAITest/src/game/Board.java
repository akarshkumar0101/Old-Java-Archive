...
public class Board {
	...
	public Board newBoard(Board inp){
		return new Board(inp);
	}
		...
		for(int x = 0; x<8 ; x++){
			for(int y=0; y <8; y++){
				...
				if(!(another.getPiece(coordinate).getClass() == getPiece(coordinate).getClass()))return false;
					...
					if(!(((ChessPiece)getPiece(coordinate)).equals((ChessPiece)(another.getPiece(coordinate)))))return false;
				}
	...
	public int getNumberOfPieces(){
		...
		for(int y =0; y<8; y++){
			for(int x=0; x < 8; x++){
				if(!(board[x][y].getSimpleName().equals("NoPiece")))result++;
			}
		}
	...
	public int getNumberOfWhitePieces(){
		...
		for(int y =0; y<8; y++){
			for(int x=0; x < 8; x++){
				if(!(board[x][y].getSimpleName().equals("NoPiece")) && board[x][y].getPlayer())result++;
			}
		}

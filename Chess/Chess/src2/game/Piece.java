...
public abstract class Piece {
	 ...
	 * @return
	 */
	public static boolean pieceCanMove(Board board, int[]... move) {
		if (!Board.isInBoard(move[1]))
			return false;

		int piece = board.get(move[0]);

		if (piece == NONE)
			return false;

		else if (Utility.areSameCoor(move))
			return false;

		// if (Math.abs(board.get(move[1])) != WK) return true;

		else if (areSameSide(board.get(move[0]), board.get(move[1])))
			return false;
	...
	private static boolean kCanMove(Board board, int[][] move) {
		int dx = Math.abs(move[1][0] - move[0][0]);
		int dy = Math.abs(move[1][1] - move[0][1]);
		if (dx > 1 || dy > 1)
			return false;
		else
			return true;
	...
	private static boolean qCanMove(Board board, int[][] move) {
		return (rCanMove(board, move) || bCanMove(board, move));
	}

	private static boolean rCanMove(Board board, int[][] move) {
		...
		for (int i = 1; i < Math.max(Math.abs(dx), Math.abs(dy)); i++) {
			if (board.get(xch * i + move[0][0], ych * i + move[0][1]) != NONE)
				return false;
		}
		return true;
	}

	private static boolean nCanMove(Board board, int[][] move) {
		int dx = Math.abs(move[1][0] - move[0][0]);
		int dy = Math.abs(move[1][1] - move[0][1]);
	...
	private static boolean bCanMove(Board board, int[][] move) {
		int dx = move[1][0] - move[0][0];
		int dy = move[1][1] - move[0][1];
	...
	private static boolean pCanMove(Board board, int[][] move) {
		int piece = board.get(move[0]);
		...
		if (dx == 0 && dy * sim == 1 && board.get(move[1]) == NONE)
			return true;
		else if (dy * sim == 1 && Math.abs(dx) == 1 && areOppSide(piece, board.get(move[1])))
			return true;

		boolean moved = !(piece > 0 ? (move[0][1] == 1) : (move[0][1] == Board.WIDTH - 2));

		if (!moved && (dx == 0 && dy * sim == 2) && board.get(move[0][0], move[0][1] + sim) == NONE
				&& board.get(move[0][0], move[0][1] + 2 * sim) == NONE)
			return true;
		else
			return false;
	}

	public static boolean kCanCastle(Board board, int[][] move) {
		return false;
	}

...
public class TestingFrame extends ChessGameUI {
	...
	public void click(int... coor) {
		if (from == null) {
			...
			if (board.playerCanMakeMove(from[0], from[1], coor[0], coor[1])) {
				board.doMove(new Move(from[0], from[1], coor[0], coor[1]));
				from = null;
				revalidateFrame();
				frame.repaint();
			} else {
	...
	private class PieceLabel extends JComponent {
		...
		public void paint(Graphics g) {
			...
			if (from != null && possibleMoves.contains(new Move(from, coor))) {
	...
	private final JPanel contentPane = new JPanel() {
		...
		public void paint(Graphics g) {
			...
			for (int i = 0; i < possibleMoves.size(); i++) {
				...
				if (board.get(move.getFrom()) > 0) {
					g.setColor(Color.red);
				} else {
					g.setColor(Color.blue);
				}
				g.drawLine(move.fx() * getWidth() / 8 + halfxSquare + ((int) (shift[0] * getWidth())),
						(7 - move.fy()) * getHeight() / 8 + halfySquare + ((int) (shift[1] * getHeight())),
						move.tx() * getWidth() / 8 + halfxSquare + ((int) (shift[2] * getWidth())),
						(7 - move.ty()) * getHeight() / 8 + halfySquare + ((int) (shift[3] * getHeight())));
			}
		}
	};

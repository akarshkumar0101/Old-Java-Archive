...
public class MathParser {
	...
	public static Entity[] parse(List<Object> parts, Variable... variables) {

		recurseInternalEntities(parts, variables);

		if (parts.contains(SEPERATOR))
			return getParameters(parts, variables);

		recognizeFunctions(parts);

		recognizeAndImplementConstants(parts);
		recognizeAndImplementVariables(parts, variables);

		implementFunctions(parts);

		implementBasicFunctions(parts);
	...
	private static Entity[] getParameters(List<Object> parts, Variable... variables) {
		...
		for (int j = 0; j < num; j++) {
			...
			params[j] = parse(paramParts, variables)[0];
		}
		return params;
	}

	private static void recurseInternalEntities(List<Object> parts, Variable... variables) {
		for (int i = 0;; i++) {
			if (i >= parts.size()) {
				break;
			}
			if (parts.get(i).equals(OPEN_PAR)) {
				...
				Entity[] internalEntity = parse(parParts, variables);
	...
	private static void recognizeFunctions(List<Object> parts) {
		for (int i = 0;; i++) {
			if (i >= parts.size()) {
				break;
			}
			if (isAlphabeticCharacter(parts.get(i))) {
				for (int endIndex = parts.size(); endIndex > i; endIndex--) {
					...
					if (funcName != null && Function.isFunction(funcName)) {
	...
	private static void recognizeAndImplementConstants(List<Object> parts) {
		for (int i = 0;; i++) {
			if (i >= parts.size()) {
				break;
			}
			if (isAlphabeticCharacter(parts.get(i)) || isNumericCharacter(parts.get(i))) {
				for (int endIndex = parts.size(); endIndex > i; endIndex--) {
					...
					if (cnstStr != null) {
						Constant cnst = Constant.createConstant(cnstStr);
						if (cnst != null) {
							// found function string
							Lists.removeSubList(parts, i, endIndex);
							parts.add(i, cnst);
	...
	private static void recognizeAndImplementVariables(List<Object> parts, Variable... variables) {
		for (int i = 0;; i++) {
			if (i >= parts.size()) {
				break;
			}
	...
	private static void implementFunctions(List<Object> parts) {
		for (int i = 0;; i++) {
			if (i >= parts.size()) {
				break;
			}
			if (parts.get(i) instanceof String && Function.isFunction((String) parts.get(i))) {
				Object param = parts.get(i + 1);
				Function func = null;
				if (param instanceof Entity[]) {
					func = Function.createFunction((String) parts.get(i), (Entity[]) param);
				} else {
					func = Function.createFunction((String) parts.get(i), (Entity) param);
				}
	...
	private static void implementBasicFunctions(List<Object> parts) {
		for (Class<? extends BasicFunction> funcClass : BasicFunction.basicFunctionsOrderOfOperations) {
			implementBasicFunction(parts, BasicFunction.TOKENS.get(funcClass));
		}
	}

	private static void implementBasicFunction(List<Object> parts, Character token) {
		for (int i = 0;; i++) {
			if (i >= parts.size()) {
				break;
			}
			if (token.equals(parts.get(i))) {
				...
				Function basicFunc = Function.createFunction(String.valueOf(token), e1, e2);
				parts.remove(i + 1);
				parts.remove(i);
				parts.remove(i - 1);

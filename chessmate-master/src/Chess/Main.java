...
public class Main extends JFrame
		implements Runnable, MouseListener, MouseMotionListener, ActionListener, ChangeListener {
	/**
	 * Our version counter
	 */
	 ...
	 * 25ms.
	 */
	private boolean bRedraw = true; // the board will be redrawn when this is
									// true
	...
	public void alert(String title, String message) {
		...
		JOptionPane.showOptionDialog(this, label, title, JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE,
	...
	public void playerMoved(boolean player, ChessMove move) {
		...
		if (chess.bestMoveEval >= 1000.0f) {
			field_Score.setText("Mate in " + (chess.maxDepth - 1 - (int) (chess.bestMoveEval / 1000.0f) / 2));
		...
		/*
		 * chess.PROGRAM = !chess.PROGRAM; chess.HUMAN = !chess.HUMAN; if (
		 * chess.maxDepth == 6 ) chess.maxDepth = 4; else { chess.maxDepth = 6;
		 * }
		 * 
		 * System.out.println( (chess.PROGRAM ? "White" : "Black") +
		 * " is playing at " + chess.maxDepth );
		 * 
		 * aiCaller.go();
		 */
	...
	public void LoadImages() {
		...
		// Image strip = getImage( getCodeBase(), "../images/alpha.gif" ); //
		// applet stuff

		// SHOULD BE LOADED BY -Toolkit.getDefaultToolkit().getImage(new
		// String("../images/icon_setupboard.gif")); (not working)
		Image strip = Toolkit.getDefaultToolkit()
				.getImage(new String("C:/Users/akars/Documents/Eclipse Projects/chessmate-master/images/alpha.gif"));
		tracker.addImage(strip, 0);
		++trackerCount;
		/*
		 * // Applet stuff toolbarImages[0] = getImage( getCodeBase(),
		 * "../images/icon_new.gif" ); toolbarImages[1] = getImage(
		 * getCodeBase(), "../images/icon_loadgame.gif" ); toolbarImages[2] =
		 * getImage( getCodeBase(), "../images/icon_savegame.gif" );
		 * toolbarImages[3] = getImage( getCodeBase(),
		 * "../images/icon_takeback.gif" ); toolbarImages[4] = getImage(
		 * getCodeBase(), "../images/icon_setupboard.gif" ); toolbarImages[5] =
		 * getImage( getCodeBase(), "../images/icon_help.gif" );
		 */
		toolbarImages[0] = Toolkit.getDefaultToolkit()
				.getImage(new String("C:/Users/akars/Documents/Eclipse Projects/chessmate-master/images/icon_new.gif"));
		toolbarImages[1] = Toolkit.getDefaultToolkit().getImage(
				new String("C:/Users/akars/Documents/Eclipse Projects/chessmate-master/images/icon_loadgame.gif"));
		toolbarImages[2] = Toolkit.getDefaultToolkit().getImage(
				new String("C:/Users/akars/Documents/Eclipse Projects/chessmate-master/images/icon_savegame.gif"));
		toolbarImages[3] = Toolkit.getDefaultToolkit().getImage(
				new String("C:/Users/akars/Documents/Eclipse Projects/chessmate-master/images/icon_takeback.gif"));
		toolbarImages[4] = Toolkit.getDefaultToolkit().getImage(
				new String("C:/Users/akars/Documents/Eclipse Projects/chessmate-master/images/icon_setupboard.gif"));
		toolbarImages[5] = Toolkit.getDefaultToolkit().getImage(
				new String("C:/Users/akars/Documents/Eclipse Projects/chessmate-master/images/icon_help.gif"));
	...
	public void LoadGame() {
		...
		dlgButt_Delete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					PreparedStatement pstmt = db.dataBase.connection
							.prepareStatement("DELETE FROM Games WHERE ID=" + og.id);
					pstmt.execute();
				} catch (SQLException ex) {
	...
	public void SaveGame() {
		...
		JLabel gameDescLabel = new JLabel("Game Description: ", SwingConstants.RIGHT);
		...
		if (JOptionPane.showOptionDialog(this, savePanel, SaveTitle, JOptionPane.DEFAULT_OPTION,
				JOptionPane.QUESTION_MESSAGE, null, SaveOptionNames, SaveOptionNames[0]) == 0) {
			...
			try {
				...
				PreparedStatement selPS = db.dataBase.connection
						.prepareStatement("SELECT * FROM Games WHERE Description=?");
				...
				if (rs.next()) {
					...
					pstmt = db.dataBase.connection
							.prepareStatement("UPDATE Games SET Description=?,Position=? WHERE ID=?");
					pstmt.setInt(3, rs.getInt("ID"));

					// moveStmt =
					// db.dataBase.connection.prepareStatement("DELETE FROM
					// Moves WHERE ID="+rs.getInt("ID"));
					...
					pstmt = db.dataBase.connection
							.prepareStatement("INSERT INTO Games (Description,Position) VALUES(?,?)");
				}
				 ...
				 * ChessMove saveMove;
				 * 
				 * for ( int i = 0; i < moveList.size(); i++ ) { pstmt =
				 * db.dataBase.connection.prepareStatement(
				 * "INSERT INTO Moves (Game,From,To) VALUES(?,?,?)");
				 * pstmt.setString( 1,
				 * "SELECT FROM Games.ID WHERE Description='"
				 * +gameDescField.getText()+"'"); saveMove =
				 * (ChessMove)moveList.get(i);
				 * 
				 * pstmt.setInt( 2, saveMove.from ); pstmt.setInt( 3,
				 * saveMove.to ); pstmt.execute(); }
				 */

				/*
				 * Enumeration e = moveList.elements(); while (
				 * e.hasMoreElements() ) { moveStmt =
				 * db.dataBase.connection.prepareStatement(
				 * "INSERT INTO Moves (Game,MoveNum,Move) VALUES(?,?,?)");
				 * //moveStmt.setInt( }
				 */
	...
	public Main() {
		...
		con.setLayout(null); // We don't want a layout as we'd rather arrange
		...
		moveTable_scrollPane.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS);
		...
		// Resize the window to a very precise size that will fit on most
		// displays
		this.setSize(WINDOW_WIDTH, WINDOW_HEIGHT);

		// Determine the user name to be displayed in the title bar
		setTitle(System.getProperty("user.name") + " vs. Computer");
		setVisible(true);
		 ...
		 */
		/*
		 * timer.schedule( new TimerTask() { public void run() { if ( bRedraw )
		 * repaint(); } }, 0, 250 );
		 */
		...
		timer.schedule(new TimerTask() {
			...
			public void run() {
				...
				if (chess.bThinking) {
					++moveTime; // since this function is called every second,
								// just increment the move timer
		...
		/*
		 * if ( chess.PROGRAM == chess.WHITE ) { aiCaller.go(); bFlipBoard =
		 * !bFlipBoard; }
		 */
		NewGame();
	}
	...
	public void update(Graphics g) {
		...
		paint(gr); // Passes our off-screen buffer to our paint method, which,
					// unsuspecting, paints on it just as it would on the
					// Graphics
	...
	public static void drawPosition(Graphics g, int xOffset, int yOffset) {
		...
		for (int y = 0; y < 8; y++) {
			for (int x = 0; x < 8; x++) {
				...
				// Determine piece value stored at board square, depending on
				...
				if (bBlueScreen) // don't draw a last move if visual thinking
									// enabled.
				{
				...
				// If the square is not blank, determine correct piece image and
				// draw it.
				if (tileVal != ChessPosition.BLANK) {
					...
					// If this piece is the piece currently picked up, don't
					// draw it (it is drawn later)
					if (square + 1 == hoverPiece) // remember the + 1 for
													// indexing
					{
		...
		if (hoverPiece > 0) {
			g.drawImage(images[imgHover], mouse_x - TILE_WIDTH / 2, // x-position
																	// / -
																	// HORZ_OFFSET
		...
		// Now draw a stroke to the right of the board to indicate whose turn it
		// is
	...
	public void ShowHelpDialog() {
		...
		// JEditorPane jep = new
		// JEditorPane("text/html","http://localhost/FreshCode/index.html");
		// con.add(jep);
	...
	public void run() {
		...
		while (!bQuit) {
			...
			/*
			 * ++timeTick; if ( timeTick >= 20 ) { timeTick = 0; if ( !bRedraw )
			 * repaint();
			 * 
			 * if ( chess.bThinking ) { field_NodesSecond.setText("" + 2 *
			 * (chess.nodeCount - nodesSecond)); nodesSecond = chess.nodeCount;
			 * 
			 * field_Nodes.setText(""+chess.nodeCount);
			 * field_Depth.setText(chess.maxDepth + "/" + chess.reachedDepth); }
			 * else nodesSecond = 0; }

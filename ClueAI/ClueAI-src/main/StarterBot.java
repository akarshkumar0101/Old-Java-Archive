...
//java -jar Clue.jar "java -jar Human.jar Name" "java -jar StarterBot.jar"
...
public class StarterBot {
	...
	public boolean play() {
		...
		while (scan.hasNextLine()) {
			...
			if (data[0].equals("ID")) {
				...
				if (type == 0) {
					knownChars.addKnown(val);
				} else if (type == 1) {
					knownWeaps.addKnown(val);
				} else {
					knownRooms.addKnown(val);
				}
				...
				if (wasGuess) {
					...
					int y = (int) (Math.random() * (range - Math.abs(x))) * (Math.random() >= 0.5 ? -1 : 1) + cur[0]; // move
																														// to
																														// a
																														// random
																														// pos
				...
				// TODO THIS IS WHERE YOU MAKE YOUR BOT DETERMINE WHICH CARD TO
				// TELL THE ENGINE THAT IT HAS
				if (knownChars.getKnown().contains(p)) {
					System.out.println(0);
				} else if (knownWeaps.getKnown().contains(w)) {
					System.out.println(1);
				} else if (knownRooms.getKnown().contains(r)) {
	 ...
	 * @param pLocs
	 *            the 2D array representing the player locations
	...
	public static void main(String[] args) {
		if (args.length > 0) {
			try {
				...
				// Use the System.err printstream for debugging since the engine
				// captures System.out.
